import { formatBodyType, formatColour, formatFuelType, formatNumber, formatOdometerUnit, formatPrice, formatTransmission, formatUlezCompliance } from "@/lib/utils";
			...
			<div className="flex flex-col md:flex-row">
				...
				<div className="md:w-1/2 md:pl-8 mt-4 md:mt-0">
					<div className="flex flex-col md:flex-row items-start md:items-center">
						...
						/>
						<div>
							<h1 className="text-gray-700 text-2xl md:text-3xl font-bold">{props.title}</h1>
						</div>
					</div>
					...
					<div className="text-gray-700 text-4xl font-bold my-4 w-full border border-slate-200 flex justify-center items-center rounded-xl py-12">
						Our Price:{" "}
					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
						{features(props).map(({ id, icon, label }) => (
							...
							>
								{icon}
								<p className="text-gray-600 text-sm font-semibold mt-2">{label}</p>
							</div>
						))}
					</div>
				</div>
